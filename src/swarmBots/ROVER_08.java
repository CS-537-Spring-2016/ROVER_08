...
public class ROVER_08 {
	...
	public ROVER_08() {
		...
		sleepTime = 150; // in milliseconds - smaller is faster, but the server
		...
		try {
			...
			while (true) {
				...
				if (line.startsWith("TIMER")) {
					...
					System.out.println(rovername + " timeRemaining: "
							+ timeRemaining);
				}
				...
				if (blocked) {
					directionChecker();
					/*
					 * for (int i = 0; i < 5; i++) { out.println("MOVE E");
					...
					if (goingSouth) {
						...
						// (scanMap may be old data by now)
						// System.out.println("ROVER_08 scanMapTiles[2][1].getHasRover() "
						// + scanMapTiles[2][1].getHasRover());
						// System.out.println("ROVER_08 scanMapTiles[2][1].getTerrain() "
				...
				System.out
						.println("ROVER_08 ------------ bottom process control --------------");
			}
	...
	private ArrayList<String> getEquipment() throws IOException {
		...
		// System.out.println("ROVER_08 incomming EQUIPMENT result - first readline: "
		// + jsonEqListIn);
		...
		if (jsonEqListIn.startsWith("EQUIPMENT")) {
			while (!(jsonEqListIn = in.readLine()).equals("EQUIPMENT_END")) {
				...
				// System.out.println("ROVER_08 incomming EQUIPMENT result: " +
				// jsonEqListIn);
	...
	public void doScan() throws IOException {
		...
		System.out.println("ROVER_08 incomming SCAN result - first readline: "
				+ jsonScanMapIn);

		if (jsonScanMapIn.startsWith("SCAN")) {
			while (!(jsonScanMapIn = in.readLine()).equals("SCAN_END")) {
				// System.out.println("ROVER_08 incomming SCAN result: " +
				// jsonScanMapIn);
	...
	public void getTargetDirection(Coord currentLoc) throws Exception {
		...
		if (currentLoc.xpos == targetLoc.xpos
				&& currentLoc.ypos == targetLoc.ypos) {
			directionChecker();
